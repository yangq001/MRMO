...
for(i in 1:p){
  for(j in 1:p){
    rhog[i,j]=hap^(abs(j-i))
  }
}
set.seed(1)
maf=runif(p,0.3,0.5)
Gall1=rmvbin(n*2*iter,maf,bincorr=rhog)
Gall2=rmvbin(n*2*iter,maf,bincorr=rhog)
Gall=Gall1+Gall2
rm(Gall1)
rm(Gall2)
...
nono=paste("MRMO_sim_AR ",q1,"_",q2," iter",iter,"_n",n,"_cor",mincor,"_",maxcor,"_size",paste(b, collapse = "_"),".csv",sep="")
